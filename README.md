...
### Поиск по reference catalogue

Поиск эталонов работает с полной кривой активного спектра, а не с таблицей
пиков, поэтому его кнопка находится рядом с вкладками спектров. Результаты
сохраняются отдельно для каждого `spectrumId` в локальной сессии.

- все результаты находятся в выезжающей правой панели, не растягивая рабочую
  область диаграммы;
- совпадения с cosine similarity `≥ 60%` сразу получают название и формулу из
  кэшируемого PubChem lookup;
- менее похожие гипотезы находятся в сворачиваемой секции и получают название
  только после явного клика;
- SMILES преобразуется в изображение структуры прямо в браузере;
- ссылка `Search structure in PubChem` открывает поиск структуры, чтобы найти
  возможное название вещества, синонимы и идентификаторы.

Исходный Zenodo-набор не содержит названий веществ: в его Parquet-файлах есть
только числовой `id`, SMILES, ось и рассчитанная IR-кривая. Поэтому совпадение
остаётся гипотезой. PubChem-метаданные — внешний справочный слой, а не часть
исходного набора и не доказательство идентификации.
...
| [SmilesDrawer](https://github.com/reymond-group/smilesDrawer) | browser-отрисовка структур из SMILES | MIT |
